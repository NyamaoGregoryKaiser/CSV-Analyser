...
int compareEarthquakes(const void* a, const void* b);
...
int main() {
    ...
    // Sort earthquakes in descending order by cost (magnitude * $1,000,000)
    qsort(earthquakes, count, sizeof(Earthquake), compareEarthquakes);
...
void readCSV(const char* filename, Earthquake** earthquakes, int* count) {
    ...
    *earthquakes = (Earthquake*)malloc(capacity * sizeof(Earthquake));
    ...
    while (fgets(line, MAX_LINE_LENGTH, file)) {
        ...
        // Extract the 'mag' (magnitude) field
        for (int i = 0; i < 4; ++i) {
            token = strtok(NULL, ",");
        }
        eq.mag = atof(token);
        ...
        // Extract the 'place' field (assume it's the 14th column in the CSV)
        for (int i = 0; i < 9; ++i) {
            token = strtok(NULL, ",");
        }
        if (token != NULL) {
...
int compareEarthquakes(const void* a, const void* b) {
    Earthquake* eq1 = (Earthquake*)a;
    Earthquake* eq2 = (Earthquake*)b;
    return (eq2->cost - eq1->cost);
}
